...
type System = { mesh_ready: boolean; reconstruction_ready: boolean; backend: string; engines: Record<string, boolean>; single_photo?: { ready: boolean; installed: boolean; message: string; model: string; device: string; download_bytes: number; download_total: number; download_state: string } };
...
async function api<T = Job>(path: string, init?: RequestInit): Promise<T> {
  const headers = new Headers(init?.headers);
  headers.set('X-PhotoForm-Client', 'local');
  const response = await fetch(path, { ...init, headers });
  const value = await response.json() as T & { detail?: unknown };
  if (!response.ok) throw new Error(typeof value.detail === 'string' ? value.detail : '请求失败，请检查参数。');
  return value;
}
...
export default function Home() {
  ...
  useEffect(() => {
    api<System>('/api/system').then(setSystem).catch(() => setError('本地服务未连接，请使用“启动应用.cmd”启动。'));
    ...
    api<Job[]>('/api/jobs').then((jobs: Job[]) => {
      ...
      if(pick) api('/api/jobs/'+pick.id).then(next => {
        if(selectionVersion.current === version) setJob(next);
      }).catch(()=>{});
    }).catch(()=>{});
  }, []);
  useEffect(() => {
    if (system?.single_photo?.ready) return;
    const timer = setInterval(() => { void api<System>('/api/system').then(setSystem).catch(() => {}); }, 8000);
  ...
  useEffect(() => {
    ...
    const version = selectionVersion.current;
    let stopped=false;
    const timer=setInterval(() => api('/api/jobs/'+job.id).then((next: Job) => { if(!stopped && selectionVersion.current === version) { setJob(next); if(!['queued','running'].includes(next.state)) refreshHistory(); } }).catch(() => { if(!stopped) setError('任务状态连接中断，请确认本地应用仍在运行。'); }), 1800);
  ...
  const selectJob = async (id: string) => {
    ...
    try { const next=await api('/api/jobs/'+id); if(selectionVersion.current===version) setJob(next); }
  ...
  return <div className="app-shell">
    <header className="topbar"><div className="brand"><span className="brand-icon"><Box size={24}/></span><div>PhotoForm <span className="brand-tag">LOCAL</span><small>照片 · 实体 · 打印</small></div></div>
      <div className="header-right"><span className="local-badge"><span/>本机处理</span><button className="quiet-button" onClick={()=>setGuide(true)}><Info size={16}/>拍摄与打印指南</button></div></header>
    <nav className="steps" aria-label="建模流程">{['导入照片 / 网格','重建与自动分色','检查模型并导出'].map((t,i)=><div className={i===(done?2:running?1:0)?'step active':'step'} key={t}><span>{i+1}</span>{t}{i<2&&<ChevronRight size={16}/>}</div>)}<span className="steps-note">STL / 3MF 工作台</span></nav>
    <main className="workspace">
      <aside className="panel source-panel"><div className="panel-title"><div><span className="eyebrow">01 / CAPTURE</span><h2>素材</h2></div><ImagePlus size={20}/></div>
        ...
        {kind==='single'&&system?.single_photo&&!system.single_photo.ready&&<p className="hint">{system.single_photo.installed?'环境已安装，正在检验实际生成效果和打印导出。':`官方权重：${Math.min(100,Math.floor(system.single_photo.download_bytes/system.single_photo.download_total*100))}% · 安装完成并验证后开放生成。`}</p>}
        {kind==='photos'&&system&&!system.reconstruction_ready&&<p className="hint">运行应用文件夹中的「安装重建引擎.ps1」。已有网格的分色导出可单独使用。</p>}
        <div className="history"><h3>本地任务 <span>{history.length}</span></h3>{history.length===0?<p className="hint">生成的任务保存在本机，可在这里重新打开。</p>:history.slice(0,6).map(h=><button disabled={running} onClick={()=>selectJob(h.id)} className={h.id===job?.id?'selected':''} key={h.id}><Box size={15}/><span>{kindName(h.kind)}<small>{h.id.slice(0,8)}</small></span><span className={'history-state '+h.state}>{h.state==='complete'?'完成':h.state==='failed'?'需处理':h.state==='cancelled'?'已取消':'处理中'}</span></button>)}</div>
      </aside>
      <section className="center-column"><div className="viewer">
        <div className="viewer-heading"><span><span className="status-dot ready"/>三维预览</span><span>{done?(job?.kind==='demo'?'测试模型 · 非照片重建':'分色实体'):'毫米 · Z 轴向上'}</span></div>
        <ModelViewer url={done?fileUrl('preview.glb'):undefined} exploded={exploded} wireframe={wireframe} hidden={hidden} reset={reset} unlit={unlit}/>
        {!done&&!running&&<div className="viewer-empty"><span className="empty-icon"><Layers3 size={38} strokeWidth={1.3}/></span><span className="eyebrow">本地模型工作台</span><h1>{kind==='single'?'从照片生成立体模型':'等待生成模型'}</h1><p>{kind==='single'?(system?.single_photo?.ready?'上传照片，生成形状，再分色导出。':'混元 2mini 正在安装和实测，进度显示在左侧。'):kind==='photos'?'上传同一物体的多角度照片，重建真实表面。':'导入彩色网格，生成多色打印部件。'}</p><small>生成后请结合原照片检查轮廓、眼鼻细节和颜色位置。</small></div>}
        ...
        <div className="viewer-bottom"><span>拖动旋转 · 滚轮缩放 · 右键平移</span><div><button disabled={!done} className={wireframe?'on':''} onClick={()=>setWireframe(v=>!v)} title="线框" aria-label="切换线框"><ScanLine size={17}/></button><button disabled={!done} onClick={()=>setReset(v=>v+1)} title="复位视角" aria-label="复位视角"><RotateCcw size={17}/></button></div></div>
      </div>
      ...
      <div className="model-details"><div className="detail-heading"><h2><Layers3 size={18}/>颜色与部件 <span>{parts.length||'—'}</span></h2><span className="hint">所有部件保持共同坐标</span></div>
      ...
      <aside className="panel settings-panel"><div className="panel-title"><div><span className="eyebrow">02 / FABRICATE</span><h2>生成设置</h2></div><Settings2 size={20}/></div>
        ...
        <button className="primary full generate" disabled={running||!system?.mesh_ready||(kind==='single'?!system?.single_photo?.ready||photos.length!==1:kind==='photos'?!system?.reconstruction_ready||photos.length<8:!mesh)} onClick={()=>startJob('upload')}>{running?<Loader2 className="spin" size={18}/>:<Box size={18}/>}生成{kind==='single'&&region?'选区':'分色'}模型 <ArrowRight size={17}/></button>
        {job?.source_available&&!running&&<button className="secondary full" onClick={()=>startJob('reprocess')}>用当前参数重新分色</button>}
        {job?.can_refine&&!running&&<button className="secondary full" onClick={()=>startJob('refine')}>按 {options.mesh_resolution} 精度重新提取形状</button>}
        {job?.can_resume&&!running&&<button className="secondary full" onClick={()=>startJob('resume')}>{job.raw_shape_available?'复用已生成形状，继续上色与导出':'从已保存的步骤继续'}</button>}
        <div className="export-block"><span className="eyebrow">03 / EXPORT</span><h3>带到切片软件</h3><p className="hint">分色 STL 不携带颜色。优先使用 3MF 保留部件和显示颜色。</p>
    ...
    <footer><span><HardDrive size={13}/>照片与模型保存在本机 · 不调用云端生成服务</span><span>PhotoForm 0.1 · 本地单图生成 / 实体分色</span></footer>
